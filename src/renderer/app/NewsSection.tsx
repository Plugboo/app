        ...
        <div className="flex gap-3 w-full overflow-hidden overflow-x-auto scrollbar-none h-44">
            {!loading &&
                ...
                news.map((article, index) => (
                    ...
                    >
                        <div className="w-full aspect-16/9 shrink-0 flex flex-col border-background-700 hover:border-primary-500 transition-color duration-300 border-2 rounded-lg overflow-hidden cursor-pointer group">
            ...
            {(loading || (!loading && news.length === 0)) && (
                <div className="w-full px-4 py-3 h-full flex items-center justify-center text-text-200 select-none">
                    {!loading && news.length === 0 && <p>No news available. Check back later!</p>}
