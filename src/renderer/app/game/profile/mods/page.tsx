...
export default function ModsPage() {
  ...
  const search = (force: boolean) => {
    ...
    setLoading(true)
    setLastInput(input)
    searchMods(gameId, input).then((result) => {
      setRecords(result)
      setLoading(false)
    })
  }
            ...
            <motion.div
              ...
              <Link
                ...
                <div className="w-32 h-full overflow-hidden rounded-lg shrink-0">
                  <img className="w-full h-full object-cover" src={record.media[0].url}
                ...
                <div className="flex flex-col justify-between min-w-0 overflow-hidden">
                  ...
                  <div className="flex gap-1.5 flex-none">
                    {record.tags.map((tag: string) => (
                      <p className="px-2 py-0.5 text-sm rounded-lg text-secondary-300 min-w-0 bg-secondary-600/30 grow-0 flex-none flex flex-row gap-4" key={tag}>
                        {tag}
                      </p>
                    ))}
                  </div>
