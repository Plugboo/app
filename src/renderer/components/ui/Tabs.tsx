...
export default function Tabs(props: Props) {
    const children = (Array.isArray(props.children) ? props.children : [props.children]).filter((v) => (v as any).type.name === 'Tab')
    ...
    return <div className="flex flex-col gap-4">
        <div className="relative flex bg-background-800 rounded-2xl overflow-hidden self-start">
            ...
            {children.map((v, index) => {
                ...
                return (
                    <div className="cursor-pointer p-3" key={props.title}
                         ref={(el) => {
